...
def test_layer_norm_simple_case():
    ...
    input = Variable(np.array([[[1.0, 2.0], 
                              [3.0, 4.0]]], dtype=np.float32))  # shape: (1, 2, 2)
    normalized_shape = (2,)
    weight = Variable(np.array([1.0, 1.0], dtype=np.float32))
    bias = Variable(np.array([0.0, 0.0], dtype=np.float32))
    eps = 1e-5
...
def test_layer_norm_with_scale_shift():
    ...
    normalized_shape = (2,)
    weight = Variable(np.array([2.0, 0.5], dtype=np.float32))
    bias = Variable(np.array([1.0, -1.0], dtype=np.float32))
    eps = 1e-5

    # Expected values:
    # Normalized: [[[-1.0, 1.0], [-1.0, 1.0]]]
    # After scale and shift: [[[-1.0, -0.5], [-1.0, -0.5]]]
    expected_output = np.array([[[-1.0, -0.5], 
...
def test_layer_norm_zero_mean_unit_var():
    ...
    input = Variable(np.array([[1.0, 2.0, 3.0, 4.0]], dtype=np.float32))
    normalized_shape = (4,)
    weight = Variable(np.ones(4, dtype=np.float32))
    bias = Variable(np.zeros(4, dtype=np.float32))
    eps = 1e-5
...
def test_layer_norm_backward():
    ...
    input = Variable(np.array([[1.0, 2.0]], dtype=np.float32))
    normalized_shape = (2,)
    weight = Variable(np.array([1.0, 1.0], dtype=np.float32))
    bias = Variable(np.array([0.0, 0.0], dtype=np.float32))
    eps = 1e-5
    ...
    grad_input = input.grad  # Access grad directly from the input variable instead of through creator
...
def test_layer_norm_different_shapes():
    ...
    for input_shape, norm_shape in shapes:
        ...
        weight = Variable(np.ones(norm_shape, dtype=np.float32))
        bias = Variable(np.zeros(norm_shape, dtype=np.float32))
        eps = 1e-5
